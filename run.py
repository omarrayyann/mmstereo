...
def run_inference(model, left_file, right_file):
    ...
    disparity_vis = visualization.make_cv_disparity_image(disparity[0, 0, :, :], VIS_DISPARITY)
    disparity_small_vis = visualization.make_cv_disparity_image(disparity_small[0, 0, :, :], VIS_DISPARITY // scale)
    disparity_small_vis = cv2.resize(disparity_small_vis, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
    confidence_vis = visualization.make_cv_confidence_image(torch.exp(matchability[0, 0, :, :]))
    confidence_vis = cv2.resize(confidence_vis, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--left", default="left.png", type=str,
                        help="Filename of left image to use for inference")
    parser.add_argument("--right", default="right.png", type=str,
